...
class BibleProvider extends GetConnect {
  ...
  Future<Result<List<BibleItem>>> searchMultiLines(String label, int paragraph, String chapters) async {
    final response = await get('$localUrl/searchMul/$label/$paragraph/$chapters');
    final String? body = response.bodyString;
    if (response.isOk) {
      var decode = json.decode(body!);
      return Result.success(ResponseData<BibleItemList>.fromJson(decode).response.list);
    } else {
